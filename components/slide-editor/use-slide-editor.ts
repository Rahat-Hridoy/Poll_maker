...
export function useSlideEditor(initialSlide: Slide | null) {
  ...
  const [history, setHistory] = useState<{
    past: { elements: CanvasElement[]; background: string }[];
    future: { elements: CanvasElement[]; background: string }[];
  }>({ past: [], future: [] });
  ...
  useEffect(() => {
    ...
    // CRITICAL FIX: Only load content if we are switching to a NEW slide.
    // If the slide ID is the same as what we have loaded, we must NOT overwrite local state
    // with incoming props. This prevents the "layout instability" where auto-sync/revalidation
    // wipes out work-in-progress dragging/resizing.
    ...
    try {
      ...
      if (!Array.isArray(parsed)) {
        console.warn(
          "Slide content is not an array, defaulting to empty elements",
          parsed,
        );
        parsed = [];
      }
  ...
  }, [initialSlide?.id]); // ONLY re-run when ID changes. Ignore content/background updates for same slide.

  // Update elements and save history
  const updateElements = useCallback(
    (newElements: CanvasElement[], save = true) => {
  ...
  const updateBackground = useCallback(
    (newBackground: string, save = true) => {
  ...
  const addElement = useCallback(
    (type: ElementType, initialContent?: string) => {
      const isLine =
        type === "line" || type === "arrow-line" || type.includes("wave");
  ...
  const addPollElement = useCallback(
  ...
  const addPollQRCode = useCallback(
    (shortCode: string, pollTitle: string) => {
  ...
  const updateElementAndSave = useCallback(
    (id: string, updates: Partial<CanvasElement>) => {
      updateElements(
  ...
  const handleArrange = useCallback(
    (action: "front" | "back" | "center-h" | "center-v") => {
      if (!selectedId) return;
